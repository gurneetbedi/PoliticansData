"""
Load Assam election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders — filter-first workflow supported.

Reads:
    https://en.wikipedia.org/wiki/2023_Assam_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_Assam_Legislative_Assembly_election

Assam naming: Hindi + tribal (Gondi/Halbi in Bastar region). Standard normalizer.
"""
...
WIKI_URLS = {
    2026: "https://en.wikipedia.org/wiki/2026_Assam_Legislative_Assembly_election",
    2021: "https://en.wikipedia.org/wiki/2021_Assam_Legislative_Assembly_election",
}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_assam_{year}.html"
...
def parse_with_colmap(html: str, year: int, expected: set[str]) -> list[dict]:
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update ASSAM_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Assam constituencies in DB: {len(expected)}", file=sys.stderr)
    ...
    for year in years:
        print(f"\n========== Assam {year} ==========", file=sys.stderr)
        if args.dump_tables:
            dump_tables(fetch_wiki_html(year, refetch=args.refetch))
            continue

        rp = RESULTS_DIR / f"assam_{year}_results.json"
