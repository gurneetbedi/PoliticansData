"""
Load Gujarat election results (winners + vote counts) from Wikipedia.
...
WIKI_URLS = {
    2022: "https://en.wikipedia.org/wiki/2022_Gujarat_Legislative_Assembly_election",
    2017: "https://en.wikipedia.org/wiki/2017_Gujarat_Legislative_Assembly_election",
}
...
COL_MAPS = {
    2022: GUJARAT_2022_COLS,
    2017: GUJARAT_2017_COLS,
}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_gujarat_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update GUJARAT_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Gujarat constituencies in DB: {len(expected)}", file=sys.stderr)
    if not expected and not (args.dump_tables or args.dry_run):
        sys.exit("No Gujarat constituencies in DB. "
                 "Run migrate_to_eci_only.py first, or use --dump-tables/--dry-run")
    ...
    for year in years:
        ...
        results_path = RESULTS_DIR / f"gujarat_{year}_results.json"
