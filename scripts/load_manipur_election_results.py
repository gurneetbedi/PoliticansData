"""
Load Manipur election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders.

Reads:
    https://en.wikipedia.org/wiki/2022_Manipur_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2017_Manipur_Legislative_Assembly_election

Manipuri naming: Meitei / Naga / Kuki / Zomi patterns. Names often
have suffixes like "Singh" (Meitei), "Meetei", or two-word Manipuri
compounds. Standard normalizer handles most cases.
"""
...
WIKI_URLS = {
    2022: "https://en.wikipedia.org/wiki/2022_Manipur_Legislative_Assembly_election",
    2017: "https://en.wikipedia.org/wiki/2017_Manipur_Legislative_Assembly_election",
}
...
MANIPUR_2022_COLS = {
    ...
    # Table 15 is the master results table. Layout matches Mizoram +
    # Nagaland + HP 2022 — no turnout column, single margin cell.
    # District subheader rows have 1 cell and are filtered out.
    "table_index": 15,
    "header_rows": 2,
    "cols": {
        "constituency": -12,   # 'Khundrakpam' / 'Heingang' / etc.
        "winner_name":  -11,
...
MANIPUR_2017_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2022: MANIPUR_2022_COLS, 2017: MANIPUR_2017_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_manipur_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update MANIPUR_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
    ...
    for row in data_rows:
        ...
        # When `expected` is empty (first-time state, DB not populated
        # yet), we accept all Wikipedia rows and let downstream fuzzy
        # matching handle noise. When it's populated, we filter to
        # known-good constituencies only.
        if not const_norm:
            skipped += 1
            continue
...
def main():
    ...
    print(f"Manipur constituencies in DB: {len(expected)}", file=sys.stderr)

    # --dump-tables just prints Wikipedia's structure; doesn't need DB.
    # --dry-run + --refetch also doesn't strictly need DB constituencies —
    # if the DB is empty (first-time state), the parser accepts all
    # Wikipedia rows and lets the downstream fuzzy-match against the
    # affidavit manifest handle filtering. Only the real DB-writing run
    # requires the constituency lookup.
    if not expected and not (args.dump_tables or args.dry_run):
        sys.exit("No Manipur constituencies in DB. "
    ...
    for year in years:
        ...
        results_path = RESULTS_DIR / f"manipur_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Manipur appearances for {year}",
              file=sys.stderr)
