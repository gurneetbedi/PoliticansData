"""
Load Nagaland election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders.

Reads:
    https://en.wikipedia.org/wiki/2023_Nagaland_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_Nagaland_Legislative_Assembly_election

Naga naming: often single-name conventions ("Neiphiu Rio"), Christian
first names + tribal/clan surnames, minimal honorifics. Standard
normalizer handles most cases; threshold=70 is generous for the
...
WIKI_URLS = {
    2023: "https://en.wikipedia.org/wiki/2023_Nagaland_Legislative_Assembly_election",
    2018: "https://en.wikipedia.org/wiki/2018_Nagaland_Legislative_Assembly_election",
}
...
# Table 12 is the master results table. Layout matches Mizoram
# exactly — no turnout column, single margin cell. 14-cell first
# row of district (with district cell), 13-cell subsequent rows.
NAGALAND_2023_COLS = {
    "table_index": 12,
    "header_rows": 2,
    "cols": {
        "constituency": -12,   # 'Dimapur II (ST)' / 'Dimapur III (ST)' / etc.
        "winner_name":  -11,
...
COL_MAPS = {
    2023: NAGALAND_2023_COLS,
    2018: NAGALAND_2018_COLS,
}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_nagaland_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update NAGALAND_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Nagaland constituencies in DB: {len(expected)}", file=sys.stderr)
    if not expected:
        sys.exit("No Nagaland constituencies in DB. Run migrate_to_eci_only.py first.")

    grand_total_matched = 0
    grand_total_unmatched = 0

    for year in years:
        print(f"\n========== Nagaland {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"nagaland_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Nagaland appearances for {year}",
              file=sys.stderr)
