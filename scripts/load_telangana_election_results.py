"""
Load Telangana election results (winners + vote counts) from Wikipedia.
...
Telangana naming: Hindi + tribal (Gondi/Halbi in Bastar region). Standard normalizer.
"""
...
WIKI_URLS = {
    2023: "https://en.wikipedia.org/wiki/2023_Telangana_Legislative_Assembly_election",
    2018: "https://en.wikipedia.org/wiki/2018_Telangana_Legislative_Assembly_election",
}
...
TELANGANA_2023_COLS = {
    # NE-standard 13-cell layout. Table 13 in Wikipedia's 2023 Telangana page.
    "table_index": 13,
    "header_rows": 2,
    "cols": {
        "constituency": -12,
...
TELANGANA_2018_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2023: TELANGANA_2023_COLS, 2018: TELANGANA_2018_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_telangana_{year}.html"
...
def parse_with_colmap(html: str, year: int, expected: set[str]) -> list[dict]:
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update TELANGANA_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Telangana constituencies in DB: {len(expected)}", file=sys.stderr)
    ...
    for year in years:
        print(f"\n========== Telangana {year} ==========", file=sys.stderr)
        if args.dump_tables:
            dump_tables(fetch_wiki_html(year, refetch=args.refetch))
            continue

        rp = RESULTS_DIR / f"telangana_{year}_results.json"
