"""
Load Meghalaya election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders — filter-first workflow supported
(dump-tables + dry-run work without DB constituencies loaded).

Reads:
    https://en.wikipedia.org/wiki/2023_Meghalaya_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_Meghalaya_Legislative_Assembly_election

Meghalaya naming: Khasi-Jaintia-Garo tribal names + Bengali/English
influences. Standard normalizer handles them.
"""
...
WIKI_URLS = {
    2023: "https://en.wikipedia.org/wiki/2023_Meghalaya_Legislative_Assembly_election",
    2018: "https://en.wikipedia.org/wiki/2018_Meghalaya_Legislative_Assembly_election",
}
...
MEGHALAYA_2023_COLS = {
    ...
    # Table 14 is the master results table. Layout matches the NE
    # standard (Mizoram/Nagaland/HP/Manipur/Tripura): no turnout
    # column, single margin cell. 14-cell first-row-of-district,
    # 13-cell subsequent rows. Negative indices handle both.
    "table_index": 14,
    "header_rows": 2,
    "cols": {
        "constituency": -12,   # 'Jowai (ST)' / 'Raliang (ST)' / etc.
        "winner_name":  -11,
...
MEGHALAYA_2018_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2023: MEGHALAYA_2023_COLS, 2018: MEGHALAYA_2018_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_meghalaya_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update MEGHALAYA_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
    ...
    for row in data_rows:
        ...
        if not const_norm:
            skipped += 1
            continue
        # Filter-first workflow: empty expected = accept all rows
...
def main():
    ...
    print(f"Meghalaya constituencies in DB: {len(expected)}", file=sys.stderr)

    if not expected and not (args.dump_tables or args.dry_run):
        sys.exit("No Meghalaya constituencies in DB. "
    ...
    for year in years:
        print(f"\n========== Meghalaya {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"meghalaya_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Meghalaya appearances for {year}",
              file=sys.stderr)
