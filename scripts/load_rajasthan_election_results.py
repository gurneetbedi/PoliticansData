"""
Load Rajasthan election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders — filter-first workflow supported.

Reads:
    https://en.wikipedia.org/wiki/2023_Rajasthan_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_Rajasthan_Legislative_Assembly_election

Rajasthan naming: Hindi + tribal (Gondi/Halbi in Bastar region). Standard normalizer.
"""
...
WIKI_URLS = {
    2023: "https://en.wikipedia.org/wiki/2023_Rajasthan_Legislative_Assembly_election",
    2018: "https://en.wikipedia.org/wiki/2018_Rajasthan_Legislative_Assembly_election",
}
...
RAJASTHAN_2018_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2023: RAJASTHAN_2023_COLS, 2018: RAJASTHAN_2018_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_rajasthan_{year}.html"
...
def parse_with_colmap(html: str, year: int, expected: set[str]) -> list[dict]:
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update RAJASTHAN_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Rajasthan constituencies in DB: {len(expected)}", file=sys.stderr)
    ...
    for year in years:
        print(f"\n========== Rajasthan {year} ==========", file=sys.stderr)
        if args.dump_tables:
            dump_tables(fetch_wiki_html(year, refetch=args.refetch))
            continue

        rp = RESULTS_DIR / f"rajasthan_{year}_results.json"
