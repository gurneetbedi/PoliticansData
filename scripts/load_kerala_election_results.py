"""
Load kerala election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders — filter-first workflow supported.

Reads:
    https://en.wikipedia.org/wiki/2023_kerala_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_kerala_Legislative_Assembly_election

kerala naming: Hindi + tribal (Gondi/Halbi in Bastar region). Standard normalizer.
"""
...
WIKI_URLS = {
    2026: "https://en.wikipedia.org/wiki/2026_kerala_Legislative_Assembly_election",
    2021: "https://en.wikipedia.org/wiki/2021_kerala_Legislative_Assembly_election",
}
...
KERALA_2021_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2026: KERALA_2026_COLS, 2021: KERALA_2021_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_kerala_{year}.html"
...
def parse_with_colmap(html: str, year: int, expected: set[str]) -> list[dict]:
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update KERALA_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"kerala constituencies in DB: {len(expected)}", file=sys.stderr)
    ...
    for year in years:
        print(f"\n========== kerala {year} ==========", file=sys.stderr)
        if args.dump_tables:
            dump_tables(fetch_wiki_html(year, refetch=args.refetch))
            continue

        rp = RESULTS_DIR / f"kerala_{year}_results.json"
