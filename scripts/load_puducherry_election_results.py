"""
Load Puducherry election results from Wikipedia.

Sibling to load_delhi_election_results.py and load_punjab_election_results.py
— same shape, Puducherry-specific table parser.

Reads:
    https://en.wikipedia.org/wiki/2021_Puducherry_Legislative_Assembly_election

Filters the DB candidate lookup to state='Puducherry' so a 'Murugan' in
a Tamil Nadu seat won't accidentally match a 'Murugan' in Puducherry.

Tamil naming notes:
  Puducherry candidates often appear as:
    - "M. Annibal" (initials before name)
    - "Angalane @ Deva Pozhilan" (@ marks an alias)
    - "Anna Durai. M" (initial after name)
  The normalize functions strip these — see _normalize_name below.

FIRST-RUN WORKFLOW
==================
    python scripts/load_puducherry_election_results.py --year 2021 --dump-tables

Paste the dump-tables output back; we'll tune PUDUCHERRY_2021_COLS, then
re-run without --dump-tables.
"""
...
WIKI_URLS = {
    2021: "https://en.wikipedia.org/wiki/2021_Puducherry_Legislative_Assembly_election",
    2016: "https://en.wikipedia.org/wiki/2016_Puducherry_Legislative_Assembly_election",
}
...
# Wikipedia Table 15 is the master results table. Puducherry's page uses
# district rowspan grouping — the first constituency row per district has
# an extra 'District' cell prepended (15 cells total); subsequent rows in
# the same district have 14 cells. Solution: use NEGATIVE column indices
# so both row shapes align. Python list negative indexing handles this
# natively.
PUDUCHERRY_2021_COLS = {
    "table_index": 15,
    "header_rows": 2,
    "cols": {
        "constituency": -13,   # Mannadipet / Thirubuvanai (SC) / etc.
        "winner_name":  -11,
        "winner_party":  -9,
        "winner_votes":  -8,
        "winner_pct":    -7,
        ...
        "runner_votes":  -3,
        "runner_pct":    -2,
        # -1 = margin votes (ignored)
    },
}

PUDUCHERRY_2016_COLS = {
    "table_index": None,
    "header_rows": 1,
    "cols": {},
}
...
COL_MAPS = {
    2021: PUDUCHERRY_2021_COLS,
    2016: PUDUCHERRY_2016_COLS,
}


def _normalize_name(name: str) -> str:
    """Normalize a Tamil-style or English name for matching.

    Tamil candidates often have name + initial patterns like:
      'M. Annibal' / 'Annibal. M' / 'Annibal M.'
    We strip leading/trailing single-letter tokens with dots, and aliases.
    """
    if not name:
        return ""
    s = name.upper().strip()
    for prefix in ("DR. ", "DR ", "ADV. ", "ADV ", "ADVOCATE ",
        ...
        if s.startswith(prefix):
            s = s[len(prefix):]
    # Strip everything after the @ marker (Tamil alias separator)
    if "@" in s:
        s = s.split("@")[0].strip()
    # S/O markers
    ...
    # Parentheticals
    s = re.sub(r"\([^)]*\)", "", s)
    # Drop trailing single-letter-with-dot tokens like "ANNA DURAI. M" → "ANNA DURAI"
    # Iteratively in case there are multiple ("X. Y.").
    while True:
        m = re.match(r"^(.*?)\s+[A-Z]\.\s*$", s)
        if not m:
            break
        s = m.group(1).strip()
    # Drop leading single-letter-with-dot tokens like "M. ANNIBAL" → "ANNIBAL"
    while True:
        m = re.match(r"^[A-Z]\.\s+(.+)$", s)
        if not m:
            break
        s = m.group(1).strip()
    # Collapse runs of non-alphanumeric to single space
    s = re.sub(r"[^A-Z0-9]+", " ", s)
    return s.strip()
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_puducherry_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update PUDUCHERRY_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
    ...
    # Minimum row length required: for positive indices we need
    # len(cells) > max_positive; for negative indices we need
    # len(cells) >= abs(min_negative). Handle mixed sets by computing both.
    ...
    for row in data_rows:
        ...
        if len(cells) < min_len:
            ...
            continue
        try:
            # Python list indexing handles negatives natively —
            # cells[-13] gets the 13th-from-end cell in either
            # a 14-cell or 15-cell row.
...
def main():
    ...
    years = [args.year] if args.year else [2021, 2016]
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    ...
    print(f"Puducherry constituencies in DB: {len(expected)}",
          file=sys.stderr)
    if not expected:
        sys.exit("No Puducherry constituencies in DB. Run "
                 "migrate_to_eci_only.py with Puducherry data first.")

    grand_total_matched = 0
    grand_total_unmatched = 0

    for year in years:
        print(f"\n========== Puducherry {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"puducherry_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Puducherry appearances for {year}",
              file=sys.stderr)
