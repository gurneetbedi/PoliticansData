"""
Load Uttarakhand election results (winners + vote counts) from Wikipedia.
...
Reads:
    ...
    https://en.wikipedia.org/wiki/2017_Uttarakhand_Legislative_Assembly_election

Uttarakhand naming: standard North Indian hill-state patterns
(Rajput, Brahmin, Sikh in Terai belt). Standard normalizer handles.
"""
...
WIKI_URLS = {
    2022: "https://en.wikipedia.org/wiki/2022_Uttarakhand_Legislative_Assembly_election",
    2017: "https://en.wikipedia.org/wiki/2017_Uttarakhand_Legislative_Assembly_election",
}
...
UTTARAKHAND_2022_COLS = {
    # Filled in after --dump-tables diagnostic on 2026-06-27.
    # Table 20 is the master results table. Layout matches Puducherry
    # 2021 / Sikkim 2019 / Haryana 2024 — has a Turnout% column, 14-cell
    # rows, single margin cell. District subheader rows have 1 cell
    # and get filtered out.
    "table_index": 20,
    "header_rows": 2,
    "cols": {
        "constituency": -13,   # 'Purola (SC)' / 'Yamunotri' / etc.
        # -12 = turnout % (ignored)
        "winner_name":  -11,
...
UTTARAKHAND_2017_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2022: UTTARAKHAND_2022_COLS, 2017: UTTARAKHAND_2017_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_uttarakhand_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update UTTARAKHAND_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
    ...
    for row in data_rows:
        ...
        if not const_norm:
            skipped += 1
            continue
        # Filter-first support: empty expected = accept all rows
...
def main():
    ...
    print(f"Uttarakhand constituencies in DB: {len(expected)}", file=sys.stderr)

    if not expected and not (args.dump_tables or args.dry_run):
        sys.exit("No Uttarakhand constituencies in DB. "
    ...
    for year in years:
        print(f"\n========== Uttarakhand {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"uttarakhand_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Uttarakhand appearances for {year}",
              file=sys.stderr)
