"""
Load Mizoram election results (winners + vote counts) from Wikipedia.

Sibling to the Delhi / Punjab / Puducherry / Goa / Sikkim loaders.

Reads:
    https://en.wikipedia.org/wiki/2023_Mizoram_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_Mizoram_Legislative_Assembly_election

Mizoram naming notes:
  - Mizo names often have single-word conventions ("Lalduhoma", "Lalthanhawla")
  - Some candidates use Christian-first-name + Mizo-surname patterns
...
WIKI_URLS = {
    2023: "https://en.wikipedia.org/wiki/2023_Mizoram_Legislative_Assembly_election",
    2018: "https://en.wikipedia.org/wiki/2018_Mizoram_Legislative_Assembly_election",
}
...
# Wikipedia Table 11 is the master results table. Mizoram's layout
# differs from Goa/Puducherry: NO turnout column, and margin is a
# single cell (no margin %). Shifts all offsets by 1 vs Goa. Uses
# negative indices so 13-cell continuation rows and 14-cell
# first-row-of-district rows both align.
MIZORAM_2023_COLS = {
    "table_index": 11,
    "header_rows": 2,
    "cols": {
        "constituency": -12,   # 'Dampa' / 'Mamit' / etc.
        "winner_name":  -11,
...
COL_MAPS = {
    2023: MIZORAM_2023_COLS,
    2018: MIZORAM_2018_COLS,
}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_mizoram_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update MIZORAM_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    ap.add_argument("--year", type=int, default=0,
                    help="2023 or 2018. Default: both.")
    ...
    print(f"Mizoram constituencies in DB: {len(expected)}", file=sys.stderr)
    if not expected:
        sys.exit("No Mizoram constituencies in DB. Run migrate_to_eci_only.py first.")

    grand_total_matched = 0
    grand_total_unmatched = 0

    for year in years:
        print(f"\n========== Mizoram {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"mizoram_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Mizoram appearances for {year}",
              file=sys.stderr)
