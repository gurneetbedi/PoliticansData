"""
Load Goa election results (winners + vote counts) from Wikipedia.

Sibling to Puducherry / Punjab / Delhi loaders. Same shape,
Goa-specific table parser.

Reads:
    https://en.wikipedia.org/wiki/2022_Goa_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2017_Goa_Legislative_Assembly_election

Filters DB candidate lookup to state='Goa' so a 'Sardesai' in Goa doesn't
merge with a 'Sardesai' in Maharashtra later.

Portuguese-influenced Goan naming: candidates often have full Portuguese
names (Digambar Kamat, Michael Lobo) or Portuguese-Konkani hybrids.
The base normalize function handles most cases; edge cases can be
threshold-tuned via --match-threshold.

FIRST-RUN WORKFLOW
==================
    python scripts/load_goa_election_results.py --year 2022 --dump-tables

Paste the dump-tables output back; tune GOA_2022_COLS, re-run without --dump-tables.
"""
...
WIKI_URLS = {
    2022: "https://en.wikipedia.org/wiki/2022_Goa_Legislative_Assembly_election",
    2017: "https://en.wikipedia.org/wiki/2017_Goa_Legislative_Assembly_election",
}
...
# Wikipedia Table 20 is the master results table. Goa's layout is like
# Puducherry's but *without* a Turnout% column between constituency and
# winner — shifts every negative offset by 1 compared to Puducherry.
# Uses negative indices so 14-cell continuation rows and 15-cell
# district-header rows both align.
GOA_2022_COLS = {
    "table_index": 20,
    "header_rows": 2,
    "cols": {
        "constituency": -13,   # 'Pernem' / 'Bicholim' / etc.
        "winner_name":  -12,
        "winner_party": -10,
        "winner_votes":  -9,
        "winner_pct":    -8,
...
def _normalize_name(name: str) -> str:
    ...
    for prefix in ("DR. ", "DR ", "ADV. ", "ADV ", "ADVOCATE ",
        ...
        if s.startswith(prefix):
            s = s[len(prefix):]
    # Alias separators sometimes used in Goa
    ...
    for marker in (" S/O ", " D/O ", " W/O "):
        if marker in s:
            s = s.split(marker)[0]
    s = re.sub(r"\([^)]*\)", "", s)
    # Trailing/leading single-initial-with-dot tokens
    while True:
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_goa_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update GOA_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Goa constituencies in DB: {len(expected)}", file=sys.stderr)
    if not expected:
        sys.exit("No Goa constituencies in DB. Run migrate_to_eci_only.py first.")

    grand_total_matched = 0
    grand_total_unmatched = 0

    for year in years:
        print(f"\n========== Goa {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"goa_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Goa appearances for {year}",
              file=sys.stderr)
