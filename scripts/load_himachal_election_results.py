"""
Load Himachal Pradesh election results (winners + vote counts) from Wikipedia.
...
HP naming: standard North Indian patterns (Rajput surnames, Brahmin
surnames, occasional Sikh). Standard normalizer handles all cases;
...
WIKI_URLS = {
    2022: "https://en.wikipedia.org/wiki/2022_Himachal_Pradesh_Legislative_Assembly_election",
    2017: "https://en.wikipedia.org/wiki/2017_Himachal_Pradesh_Legislative_Assembly_election",
}
...
# Table 27 is the master results table. Layout matches Mizoram + Nagaland
# exactly — no turnout column, single margin cell. 14-cell first row of
# district (with district cell), 13-cell subsequent rows.
HIMACHAL_2022_COLS = {
    "table_index": 27,
    "header_rows": 2,
    "cols": {
        "constituency": -12,   # 'Churah (SC)' / 'Bharmour (ST)' / etc.
        "winner_name":  -11,
...
COL_MAPS = {
    2022: HIMACHAL_2022_COLS,
    2017: HIMACHAL_2017_COLS,
}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_himachal_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update HIMACHAL_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    print(f"Himachal Pradesh constituencies in DB: {len(expected)}", file=sys.stderr)
    if not expected:
        sys.exit("No Himachal Pradesh constituencies in DB. "
    ...
    for year in years:
        ...
        results_path = RESULTS_DIR / f"himachal_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} HP appearances for {year}",
              file=sys.stderr)
