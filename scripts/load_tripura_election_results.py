"""
Load Tripura election results (winners + vote counts) from Wikipedia.

Sibling to the other state loaders.

Reads:
    https://en.wikipedia.org/wiki/2023_Tripura_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2018_Tripura_Legislative_Assembly_election

Tripuri naming: Bengali-origin names + tribal names (Kokborok speakers).
Standard normalizer handles both cases.
"""
...
WIKI_URLS = {
    2023: "https://en.wikipedia.org/wiki/2023_Tripura_Legislative_Assembly_election",
    2018: "https://en.wikipedia.org/wiki/2018_Tripura_Legislative_Assembly_election",
}
...
TRIPURA_2023_COLS = {
    ...
    # Table 11 is the master results table. Layout matches Mizoram +
    # Nagaland + HP + Manipur — no turnout column, single margin cell.
    # District subheaders (1 cell) filtered out automatically.
    "table_index": 11,
    "header_rows": 2,
    "cols": {
        "constituency": -12,   # 'Mohanpur' / 'Bamutia (SC)' / etc.
        "winner_name":  -11,
...
TRIPURA_2018_COLS = {"table_index": None, "header_rows": 2, "cols": {}}

COL_MAPS = {2023: TRIPURA_2023_COLS, 2018: TRIPURA_2018_COLS}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_tripura_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update TRIPURA_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
    ...
    for row in data_rows:
        ...
        # Filter-first workflow support: when `expected` is empty (first-time
        # state, no DB constituencies yet), accept all Wikipedia rows and
        # let downstream fuzzy-match against the affidavit manifest filter.
        if not const_norm:
            skipped += 1
            continue
...
def main():
    ...
    print(f"Tripura constituencies in DB: {len(expected)}", file=sys.stderr)

    if not expected and not (args.dump_tables or args.dry_run):
        sys.exit("No Tripura constituencies in DB. "
    ...
    for year in years:
        ...
        results_path = RESULTS_DIR / f"tripura_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Tripura appearances for {year}",
              file=sys.stderr)
