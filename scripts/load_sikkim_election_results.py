"""
Load Sikkim election results (winners + vote counts) from Wikipedia.

Sibling to the Delhi / Punjab / Puducherry / Goa loaders. Same shape,
Sikkim-specific table parser.

Reads:
    https://en.wikipedia.org/wiki/2019_Sikkim_Legislative_Assembly_election
    https://en.wikipedia.org/wiki/2024_Sikkim_Legislative_Assembly_election

Filters DB candidate lookup to state='Sikkim' so a candidate in another
state with a similar name doesn't accidentally match.

Sikkimese naming notes:
  - Names often carry ethnic prefixes/suffixes ("Bhutia", "Lepcha",
    "Chettri") that are part of the surname, not honorifics.
  - Some candidates use single-name conventions in the Sikkimese style.
  - Not much need for special normalization — standard rules work.

FIRST-RUN WORKFLOW
==================
    python scripts/load_sikkim_election_results.py --year 2019 --dump-tables

Paste dump-tables output back; tune SIKKIM_2019_COLS, re-run without --dump-tables.
"""
...
WIKI_URLS = {
    2024: "https://en.wikipedia.org/wiki/2024_Sikkim_Legislative_Assembly_election",
    2019: "https://en.wikipedia.org/wiki/2019_Sikkim_Legislative_Assembly_election",
}
...
# Wikipedia Table 6 is the master results table. Layout is identical
# to Puducherry — 14-cell data rows with a turnout% column between
# constituency and winner. District subheader rows (1 cell) are
# filtered out automatically by the length check.
SIKKIM_2019_COLS = {
    "table_index": 6,
    "header_rows": 2,
    "cols": {
        "constituency": -13,   # 'Yoksam–Tashiding' / 'Yangthang' / etc.
        # -12 = turnout % (ignored)
        "winner_name":  -11,
...
COL_MAPS = {
    2024: SIKKIM_2024_COLS,
    2019: SIKKIM_2019_COLS,
}
...
def fetch_wiki_html(year: int, refetch: bool = False) -> str:
    cache_path = RESULTS_DIR / f"_wiki_sikkim_{year}.html"
...
def parse_with_colmap(html: str, year: int,
    ...
    if cfg.get("table_index") is None:
        print(f"  ⚠ No column map for {year} yet. Run --dump-tables first,",
              file=sys.stderr)
        print(f"    update SIKKIM_{year}_COLS in this script, then re-run.",
              file=sys.stderr)
        return []
...
def main():
    ...
    ap.add_argument("--year", type=int, default=0,
                    help="2019 or 2024. Default: both.")
    ...
    print(f"Sikkim constituencies in DB: {len(expected)}", file=sys.stderr)
    if not expected:
        sys.exit("No Sikkim constituencies in DB. Run migrate_to_eci_only.py first.")

    grand_total_matched = 0
    grand_total_unmatched = 0

    for year in years:
        print(f"\n========== Sikkim {year} ==========", file=sys.stderr)
        ...
        results_path = RESULTS_DIR / f"sikkim_{year}_results.json"
        ...
        print(f"  Candidate lookup: {len(lookup)} Sikkim appearances for {year}",
              file=sys.stderr)
